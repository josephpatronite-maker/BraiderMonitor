...
# Replaces the wire break predictor. Instead of trying to predict faults from
# signals (which are fault responses, not precursors), this simply watches for
# new hires_event files and surfaces them on the dashboard immediately.
# No false positives, no threshold tuning — if a hires event was captured,
    ...
    while True:
        ...
        try:
            ...
            for fname in sorted(new_files):
                ...
                # Read puller position from the trigger row
                puller_feet = None
                try:
...
function openQrScanner() {
  ...
  qrCodeReader
    .decodeFromConstraints(
      ...
      'qr-video',
      (result, err) => {
        // err fires on every frame with nothing decodable yet — that's
        // normal and expected, not a real failure. Only act on a hit.
